...
pub async fn run(&mut self) -> Result<TestResult, TestError> {
    ...
    fn step1_generate_credentials(&mut self) -> Result<(), TestError> {
        ...
        self.log(&format!("Using known working LavaUSD pubkey: {}", self.lava_pubkey));
    ...
    async fn step2_call_faucet(&mut self) -> Result<(), TestError> {
        ...
        self.log(&format!("BTC faucet response ({} {}): {}", btc_status.as_u16(), btc_status.canonical_reason().unwrap_or("Unknown"), btc_body));
        
        // Call LavaUSD faucet with retries
        let max_lava_attempts = 3;
        for attempt in 1..=max_lava_attempts {
            self.log(&format!("LavaUSD faucet attempt {}/{}", attempt, max_lava_attempts));
            ...
            self.log(&format!("LavaUSD faucet response ({} {}): {}", lava_status.as_u16(), lava_status.canonical_reason().unwrap_or("Unknown"), lava_body));
    ...
    fn step3_check_cli(&mut self) -> Result<(), TestError> {
        ...
        if !Path::new(cli_path).exists() {
            return Err(TestError::Process(format!("CLI not found at: {}", cli_path)));
        }
        ...
        match fs::File::create(test_file) {
            ...
            Err(e) => {
                return Err(TestError::Io(format!("Current directory is not writable: {}", e)));
            }
        }
    ...
    fn step4_create_loan(&mut self) -> Result<(), TestError> {
        ...
        if !output.status.success() {
            return Err(TestError::Process(format!("Failed to create loan: exit code {}", output.status)));
        }
...
let contract_id_opt = re.captures(&stdout)
    ...
    fn step7_get_contract_details(&mut self) -> Result<(), TestError> {
        ...
        if !output.status.success() {
            return Err(TestError::Process("Failed to get contract details".to_string()));
        }
